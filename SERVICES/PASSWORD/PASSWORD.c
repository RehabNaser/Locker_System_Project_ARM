...
static uint8 SavededPassword[10]={0};
...
void SetPassword(uint8 PasswordDigitsNumber)
{
	for(uint8 i=0;i<PasswordDigitsNumber;i++)
	{
		SavededPassword[i]=EnteredPassword[i];
	}
}

uint8 CheckPassword(uint8 PasswordDigitsNumber)
{
	for(uint8 i=0;i<PasswordDigitsNumber;i++)
	{
		if(SavededPassword[i]!=EnteredPassword[i])
		{
